...
import os
# INSIGHTS_PROJECT_ROOT = r"C:\Idea Projects\AI_Test_Assist"
INSIGHTS_PROJECT_ROOT = r"C:\AssureX\backend"

if INSIGHTS_PROJECT_ROOT not in sys.path:
    sys.path.insert(0, INSIGHTS_PROJECT_ROOT)
...
#  🔥 NEW: Suppress verbose logging during step execution
# Set to WARNING to prevent debug messages from appearing in step text
logging.getLogger("plcd_taseq").setLevel(logging.INFO)  # Only INFO and above to console
...
# NEW: Suppress httpcore connection trace logs (TCP, TLS, request/response)
logging.getLogger("httpcore").setLevel(logging.WARNING)
logging.getLogger("httpcore.connection").setLevel(logging.WARNING)
logging.getLogger("httpcore.http11").setLevel(logging.WARNING)

# NEW: Suppress async/event loop logs
...
class JiraAgent(BaseAgent):
    ...
    # Jira API setup
        ...
        self.jira_url = jira_cfg.get('url')
...
class SelectorAgentL1(BaseAgent):
    ...
    def discover_selector(self, step_text: str, context: Dict[str, Any], state: TestExecutionState) -> Dict[str, Any]:
        ...
        # 🔥 UI-safe: read pending insights from state if present
        pending_from_state = state.get("pending_insights", [])

        if pending_from_state:
            for insight in pending_from_state:
                # if insight["step"].strip().lower() == step_text.strip().lower():
                if insight["step"].lower() in step_text.lower():
                    logger.info("✓ Found pending insight from state injection")
                    return {
                        "selector": insight["selector"],
                        "confidence": insight.get("confidence", 1.0),
                        "agent": "Learning (Explicit Feedback)",
                        "reasoning": "Human feedback (UI/CLI unified)"
                    }

    # ⬇️ ONLY if no human feedback matched, continue normal flow
    # existing L1 / RAG / L2 / L3 logic continues here
    ...
    def _check_pending_insights(self, step_text: str, state: TestExecutionState) -> Optional[Dict[str, Any]]:
        """
        Check learned insights using vector search (pending ALWAYS wins).
    
        Search order:
        1. ✅ Pending insights (JSON files - HIGHEST PRIORITY - confidence boosted to 1.0)
        2. Learned insights (ChromaDB)
    """
        try:
            current_module = state.get('module', '')
        
        # 🔥 PRIORITY 1: Search pending insights FIRST
            logger.debug(f"🔍 [1/2] Searching pending insights for: '{step_text}'")
            ...
            if pending_matches:
                logger.info(f"✅ Found {len(pending_matches)} pending insight(s)")
                best_match = pending_matches[0]
                for i, match in enumerate(pending_matches, 1):
                    logger.info(f"   Match {i}:")
                    logger.info(f"     Selector: {match['selector']}")
                    logger.info(f"     Similarity: {match['similarity']:.3f}")
                    logger.info(f"     File: {match.get('file', 'N/A')}")
                    # 🔥 FIX: FORCE confidence to 1.0 for user feedback (overrides everything)
                    logger.info(f"   📌 Using pending insight: {best_match['selector']}")
                    logger.info(f"   📌 Original confidence: {best_match.get('confidence', 'N/A')}")
                    logger.info(f"   📌 Forcing confidence to: 1.0")
            
            # 🔥 FORCE CONFIDENCE TO 1.0 (always wins over ChromaDB)
                # return {
                #     'selector': best_match['selector'],
                #     'confidence': 1.0,  # ✅ ALWAYS use user feedback (highest priority)
                #     'agent': 'Learning',
                #     'source': 'pending_insights',
                #     'reasoning': f"User feedback (similarity: {best_match['similarity']:.2f})"
                # }
                # 🔥 NORMALIZE selector from user feedback
                raw_selector = best_match['selector']
                normalized_selector = self.normalize_selector(raw_selector)

                logger.info("📌 Using USER FEEDBACK selector (forced)")
                logger.info(f"   Raw selector       : {raw_selector}")
                logger.info(f"   Normalized selector: {normalized_selector}")

                return {
                    'selector': normalized_selector,   # ✅ THIS fixes UI vs CLI mismatch
                    'confidence': 1.0,                  # ✅ user feedback ALWAYS wins
                    'agent': 'Learning',
                    'source': 'pending_insights',
                    'reasoning': f"User feedback (similarity: {best_match['similarity']:.2f})"
                }
        ...
        # Priority 2: Search learned insights (ChromaDB)
            logger.debug(f"🔍 [2/2] Searching learned insights (ChromaDB)...")
            ...
            if learned_matches:
                logger.info(f"✅ Found learned insight (ChromaDB)")
                ...
                return {
                    ...
                    'agent': 'Learning',
                    'source': 'chromadb',
                    'reasoning': f"Learned selector (conf: {best_match['confidence']:.2f})"
                }
            ...
            logger.error(f"Error checking insights: {e}", exc_info=True)
            return None
    ...
    def _enhance_query_with_context(self, step_text: str, context: Dict[str, Any], state: TestExecutionState) -> str:
        ...
        try:
            ...
            step_lower = step_text.lower()
             ...
             # FIX: Exclude accordion/panel/modal interactions from navigation
            # These are UI element interactions, NOT page navigation
            ui_interaction_keywords = ['accordion', 'panel', 'modal', 'dialog', 'popup', 
                                    'expand', 'collapse', 'toggle', 'dropdown']
            is_ui_interaction = any(kw in step_lower for kw in ui_interaction_keywords)
            
            is_navigation_step = any(nav_word in step_lower for nav_word in ['navigate', 'go to', 'link']) or \
            ...
            if visible_context and is_navigation_step and not is_form_field_step and not is_ui_interaction:
                query_parts.append(f"from {' '.join(visible_context[:2])}")

            # FIX: DO NOT add page context for navigation steps!
            # Navigation = "leaving current page", so current page is NOISE
            # Only add page context for non-navigation actions (edit, click button, etc.)
            if not is_navigation_step and page_context:
    ...
    def _scrape_dom_elements(self, page: Page) -> List[Dict[str, Any]]:
        ...
        try:
            elements = page.evaluate("""
                () => {
                    const elements = document.querySelectorAll('button, div, span, input, select, a, [role="button"]');
...
class PLCDTestingAssistantSeq:
    ...
    # 🔥 FIX: Initialize ChromaDB for ALL modes (normal + rerun)
    # This MUST run in subprocess too, otherwise feedback won't be queried
        from config_loader import get_chroma_client
        import chromadb
        ...
        try:
            ...
            logger.debug(f"✅ ChromaDB initialized: {chroma_path}")
        except Exception as e:
            logger.error(f"❌ ChromaDB init failed: {e}")
            self.chroma_client = None

    # Continue with other agent initialization
...
# ------------------------------------------------------------------
        ...
        #     base_path=r"C:\Idea Projects\AI_Test_Assist\insights"
        # )
        
        # Initialize storage for pending insights
        # from insight_storage import InsightStorage
        # import json
        # storage = InsightStorage(
        #     base_path=r"C:\Idea Projects\AI_Test_Assist\insights"
        ...
        storage = InsightStorage(
            # base_path=r"C:\Idea Projects\AI_Test_Assist\insights"
            base_path=os.path.join(os.path.dirname(__file__), "insights")
        )
        pending_insights = []

        # for insight_file in storage.pending_path.glob("*.json"):
        #     try:
        #         with open(insight_file, "r", encoding="utf-8") as f:
        #             pending_insights.append(json.load(f))
        #     except Exception as e:
        #         logger.warning(f"Failed to load pending insight {insight_file}: {e}")
        if storage.pending_path.exists():
            for insight_file in storage.pending_path.glob("*.json"):
                try:
                    with open(insight_file, "r", encoding="utf-8") as f:
                        pending_insights.append(json.load(f))
        ...
        if pending_insights:
            logger.info(
                f"Loaded {len(pending_insights)} pending insights for ticket {ticket_id}"
            )
    # THIS is the key line
            state["pending_insights"] = pending_insights
        else:
            logger.info("No pending insights found at execution start")
        ...
        try:
            ...
            with sync_playwright() as playwright:
                ...
                # ===============================
# EXPORT STEP RESULTS FOR BACKEND
# ===============================
                from pathlib import Path
                # import json

                output_dir = Path("Reports/steps")
                output_dir.mkdir(parents=True, exist_ok=True)

                steps_output_path = output_dir / f"steps_{state['ticket_id']}.json"

                with open(steps_output_path, "w", encoding="utf-8") as f:
                    json.dump(state["step_results"], f, indent=2, ensure_ascii=False)

                logger.info(f"Step results exported to {steps_output_path}")
    ...
    def _capture_initial_context(self, state: TestExecutionState, page: Page):
        ...
        page.goto(self.config['web_url'], timeout=90000, wait_until='domcontentloaded')
    ...
    def _execute_steps_with_agents(
        self,
        state: TestExecutionState,
        page: Page,
        ticket_data: Dict[str, Any]
    ):
        """Execute test steps using Learning + Selector agents (INDEX-BASED, SAFE)

        Improvements:
        - Uses enumerate to ensure idx is always defined
        - Pre-populates state['step_results'] to allow indexed updates
        - Robust handling for login skips, text verification (L3), L1/L2 discovery,
          action execution and failure handling.
        - 🔥 NEW: Debug logging for feedback file detection and selector resolution
        """
    ...
    # Ensure step_results has placeholder entries for each step (safe indexed writes)
        ...
        # Save clean text into state
            try:
                state["step_results"][idx]["step_text"] = clean_step_text
            except Exception:
            # Fallback: append if indexing unexpectedly fails
                state["step_results"].append({
                    "step_number": step_number,
                    "step_text": clean_step_text,
                    "selector": None,
                    "confidence": 0.0,
                    "agent_used": None,
                    "action_type": None,
        ...
        # Display progress (truncate long text)
            step_display = clean_step_text if len(clean_step_text) <= 70 else clean_step_text[:67] + "..."
        ...
        # ------------------------------------------------------------------
            if clean_step_text.lower() in ["login", "log in", "sign in", "signin"]:
                ...
                # Ensure dashboard navigation if still on login page
                    try:
                        ...
                        logger.debug("Dashboard navigation after login-skip not needed/failed")

                    continue

            try:
            # Capture context before attempting the step
                context = self.context_agent.capture_context(page)
                self.context_agent.update_history(state, context)
                state["step_results"][idx]["context"] = context

            # ------------------------------------------------------------------
            # TEXT VERIFICATION (L3)
            # ------------------------------------------------------------------
                if self._is_text_verification_step(clean_step_text):
                    expected_text = self._extract_expected_text(clean_step_text)
                    if expected_text:
                    # Small wait for transient messages
                        wait_ms = self.config.get("wait_times", {}).get("before_text_verification", 1000)
                        if wait_ms > 0:
                            page.wait_for_timeout(wait_ms)

                        l3_result = self.selector_agent_l3.verify_text(
                            page,
                            expected_text,
                            clean_step_text,
                            state.get("last_screenshot")
                        )

                        success = l3_result.get("found", False)
                        ...
                        print(f" [{'OK' if success else 'FAILED'}] (L3)")

                        if not success:
                            state["overall_status"] = "FAILED"
                            if self.config.get("execution", {}).get("failure_handling", {}).get("fail_fast"):
                                print("\n[FAIL-FAST] Stopping execution")
                                break
                        continue

            # ------------------------------------------------------------------
            # 🔥 DEBUG LOGGING: Check for learned insights BEFORE discovery
            # ------------------------------------------------------------------
                ticket_id = state.get('ticket_id', '')
                logger.info(f"🔍 [Step {step_number}] Checking for learned insights...")
            
            # Check pending insights folder
                # pending_folder = Path("insights/pending")
                from pathlib import Path
                # pending_folder = Path(r"C:\Idea Projects\AI_Test_Assist\insights\pending")
                # pending_folder = Path(r"C:\AssureX\backend\insights\pending")
                pending_folder = Path(__file__).parent / "insights" / "pending"
                logger.info(f"DEBUG: Checking pending folder path: {pending_folder} (exists: {pending_folder.exists()})")
                if pending_folder.exists():
                    feedback_files = list(pending_folder.glob(f"{ticket_id}_step{step_number}_*.json"))
                    logger.info(f"   Found {len(feedback_files)} feedback files for this step")
                    ...
                    # 🔥 DEBUG: Print file content
                        try:
                            with open(f, 'r', encoding='utf-8') as file:
                                ...
                                logger.info(f"   📄 Content: step='{insight_data.get('step', 'N/A')}', selector='{insight_data.get('selector', 'N/A')}'")
                        except Exception as e:
                            logger.warning(f"   ⚠️ Could not read {f.name}: {e}")
                else:
            ...
            # 🔥 CRITICAL: If learned selector exists with high confidence, USE IT (no fallback)
                ...
                # If L1 gives low confidence or validation indicates not present -> try L2
                    ...
                    if not selector or (not selector_validated and confidence < 0.75) or confidence < 0.70:
                        ...
                        agent_used = "L2"
                        selector = l2_result.get("selector")
                        confidence = l2_result.get("confidence", 0.0)

                    # If L2 fails, keep L1 candidate if any, otherwise raise
                        if not selector or confidence < 0.70:
                            if l1_result.get("selector"):
                                selector = l1_result.get("selector")
                                confidence = max(confidence, l1_result.get("confidence", 0.0))
                                agent_used = "L1"
                                logger.info("  [L2 failed, falling back to L1 candidate]")
                            else:
                                raise Exception(f"No selector found (L1: {l1_result.get('confidence', 0.0):.2f}, L2: {l2_result.get('confidence', 0.0):.2f})")

            # ------------------------------------------------------------------
            # EXECUTE ACTION
            # ------------------------------------------------------------------
                action_type = self._detect_action_type(clean_step_text)
                success = self._execute_action(page, selector, action_type, clean_step_text, state)
            ...
            # 🔥 ADD VERIFICATION LOG
                logger.info(f"📊 [Step {step_number}] Final state saved:")
                logger.info(f"   Selector used: {selector}")
                logger.info(f"   Agent: {agent_used}")
                logger.info(f"   Status: {'PASSED' if success else 'FAILED'}")

                print(f" [{'OK' if success else 'FAILED'}] ({agent_used})")

            # Persist successful selector to learning store
                if success and selector:
                    try:
                        self.learning_agent.store_learned_selector(
                            clean_step_text, selector, context, confidence, 
                            source="runtime", verified=True
                        )
                    except Exception as e:
                        logger.debug(f"Failed to store learned selector: {e}")
    ...
    def _generate_artifacts(self, state: TestExecutionState) -> Dict[str, Any]:
        ...
        if state['overall_status'] == 'PASSED':
         ...
         # 🔥 NEW: Save summary to JSON file (for backend summary endpoint)
        try:
            summaries_folder = Path("Reports/summaries")
            summaries_folder.mkdir(parents=True, exist_ok=True)
        
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            summary_filename = f"summary_{state['ticket_id']}_{timestamp}.json"
            summary_path = summaries_folder / summary_filename
        
        # Save timestamped summary
            with open(summary_path, 'w', encoding='utf-8') as f:
                json.dump(summary, f, indent=2, ensure_ascii=False)
        
        # Save latest summary (overwrites previous)
            latest_summary_path = summaries_folder / f"summary_{state['ticket_id']}_latest.json"
            with open(latest_summary_path, 'w', encoding='utf-8') as f:
                json.dump(summary, f, indent=2, ensure_ascii=False)
        
            logger.info(f"✅ Summary saved: {summary_path}")
            logger.info(f"✅ Latest summary: {latest_summary_path}")
...
def collect_and_process_feedback(
    ...
    Args:
        ...
        feedback_text: Optional pre-provided feedback text (for backend calls)

    Returns:
        True if feedback was collected and processed, False otherwise
    """
    # 🔥 DEBUG logging
    print(f"\n🔥 [DEBUG] collect_and_process_feedback called")
    print(f"   feedback_text: '{feedback_text}'")
    print(f"   feedback_text is None: {feedback_text is None}")
    print(f"   feedback_text is empty: {not feedback_text}")
    ...
    try:
        ...
        # =====================================================================
        # COLLECT FEEDBACK (Interactive or from parameter)
        # =====================================================================
        if feedback_text is None:
            # INTERACTIVE MODE (manual CLI usage)
        ...
        # =====================================================================
        # 🔥 FIX: LOAD CANONICAL STEP TEXTS FROM SUMMARY
        # =====================================================================
        from pathlib import Path
        summaries_folder = Path("Reports/summaries")
        summary_path = summaries_folder / f"summary_{ticket_id}_latest.json"
        
        if not summary_path.exists():
            logger.error(f"Summary not found: {summary_path}")
            print(f"\n[ERROR] Cannot find test summary for {ticket_id}")
            return False
        
        with open(summary_path, 'r', encoding='utf-8') as f:
            summary_data = json.load(f)
        
        # Get step texts from summary (these have canonical Jira text)
        actual_step_results = summary_data.get('step_results', [])
        
        logger.info(f"Loaded {len(actual_step_results)} canonical step texts from summary")
        
        # 🔥 If summary is empty, fetch from Jira
        if not actual_step_results:
            logger.warning("Summary empty, fetching canonical steps from Jira")
            jira_agent = assistant.jira_agent or JiraAgent(config)
            ticket_data = jira_agent.parse_ticket(ticket_id)
            actual_step_results = [
                {
                    'step_number': i + 1,
        ...
        # DEBUG: Print first 3 steps to verify
        for i, step in enumerate(actual_step_results[:3], 1):
            logger.debug(f"Step {i}: {step.get('step_text', 'N/A')}")

        # =====================================================================
        # BUILD TEST CONTEXT FOR PARSER
        # =====================================================================
        test_context = {
            'ticket_id': ticket_id,
            'steps': [{'text': f'Step {i}'} for i in range(1, 100)],  # Dummy for validation
            'results': {
                'step_results': actual_step_results,
        ...
        for step_key, step_feedback in feedback_steps.items():
            ...
            # 🔥 FIX: Get step text from canonical summary results
            step_text = ""
            for step_result in actual_step_results:
                result_step_num = step_result.get('step_number') or step_result.get('step') or 0
                if result_step_num == step_num:
                    step_text = step_result.get('step_text', step_result.get('text', ''))
                    break

            # Fallback if step not found (shouldn't happen with summary)
            if not step_text:
                step_text = f"Step {step_num}"
                logger.warning(f"Step {step_num} not found in summary, using fallback text")

            # Build insight structure
            # feedback_type = step_feedback.get('feedback_type', 'failed')
            # module = results.get('module', summary_data.get('module', ''))
            
            # Build insight structure
            feedback_type = step_feedback.get('feedback_type', 'failed')
            module = results.get('module', summary_data.get('module', ''))

# 🚨 HARD FAIL if module is unknown
            # if not module or module == "Unknown":
            #     logger.error(f"❌ Feedback rejected: module could not be resolved for step {step_num}")
            #     print(f"✗ Feedback rejected for Step {step_num}: module is 'Unknown'")
            #     continue
            if module == "Unknown":
                logger.warning(
                    f"⚠️ Module unresolved for step {step_num}. "
                    f"Falling back to summary module."
                )
                summary_module = summary_data.get('module', 'Unknown')
                module = summary_module  # ← derive from loaded summary


# 🔥 FIX: Normalize selector format
            raw_selector = step_feedback.get('correct_selector', '')
            normalized_selector = _normalize_css_selector(raw_selector)

            logger.info(f"📝 Normalizing selector for Step {step_num}:")
            logger.info(f"   Raw: {raw_selector}")
...
# ... rest
            ...
            insight = {
                'step': step_text,  # ✅ CANONICAL FROM SUMMARY
                'selector': normalized_selector,  # 🔥 USE NORMALIZED SELECTOR
                'confidence': 1.0,  # 🔥 FORCE TO 1.0 FOR USER FEEDBACK
                'category': feedback_type,  # ✅ Should be 'false_positive'
                'module': module,
    # ... rest
            }
            ...
            #     logger.error(f"❌ Feedback rejected: module could not be resolved for step {step_num}")
            #     print(f"✗ Feedback rejected for Step {step_num}: module is 'Unknown'")
            #     continue  # Skip this step's feedback
            if module == "Unknown":
                logger.warning(
                    f"⚠️ Module unresolved for step {step_num}. "
                    f"Falling back to summary module."
            ...
            insight = {
                # Fields required by search functions
                'step': step_text,  # 🔥 NOW CANONICAL FROM JIRA
                'selector': step_feedback.get('correct_selector', ''),
                'confidence': 0.95,
            ...
            try:
                ...
                insight['confidence'] = 1.0  # ✅ Max confidence (overrides ChromaDB's 0.97)
                ...
                # insight['embedding'] = None
                 # 🔥 HARD FAIL: Don't save feedback without embedding
                logger.error(f"❌ Embedding generation FAILED for Step {step_num}: {e}")
                print(f"✗ Cannot save feedback for Step {step_num}: embedding generation failed")
                continue  # Skip this step's feedback
                
                
            # =====================================================================
# 🔥 FIX: Delete old feedback for this step BEFORE saving new one
# =====================================================================
            try:
                pending_folder = Path("insights/pending")
                if pending_folder.exists():
        # Find all existing feedback files for this ticket + step
                    existing_files = list(pending_folder.glob(f"{ticket_id}_step{step_num}_*.json"))
                    for old_file in existing_files:
                        old_file.unlink()
                        logger.info(f"🗑️ Deleted old feedback: {old_file.name}")
            except Exception as e:
                logger.warning(f"⚠️ Could not delete old feedback: {e}")

            # =====================================================================
            # SAVE INSIGHT TO CATEGORY FOLDER
            # =====================================================================
            # 🔥 DELETE OLD FEEDBACK FILES FOR THIS STEP
            pending_folder = storage.pending_path  # Path to insights/pending
            if pending_folder.exists():
    # Delete all old feedback files for this step (any timestamp)
                old_files = list(pending_folder.glob(f"{ticket_id}_step{step_num}_*.json"))
...
def _normalize_css_selector(selector: str) -> str:
    """
    Normalize CSS selector to proper Playwright format
    
    Examples:
        "data-attribute='value'" → "[data-attribute='value']"
        "data-test='btn'" → "[data-test='btn']"
        "[data-test='btn']" → "[data-test='btn']" (already valid)
        "button.class" → "button.class" (already valid)
    
    Args:
        selector: Raw selector string
    
    Returns:
        Properly formatted CSS selector
    """
    ...
    # Pattern: Attribute format without brackets
    # Match: data-test='value', data-attribute="value", aria-label='text'
    attribute_pattern = r'^(data-|aria-)[a-zA-Z0-9_-]+=([\'"])[^\'"]*\2$'
    
    import re
    if re.match(attribute_pattern, selector):
        # Missing brackets - add them
        return f"[{selector}]"
    
    # Already valid CSS selector (has brackets or is tag/class/id)
    return selector
...
def main():
    ...
    try:
        ...
        # Execute test (always generate artifacts and summary here)
        print("\n" + "="*80)
        print("Test Execution")
        print("="*80 + "\n")
        results = assistant.execute_test(ticket_id)

        # If --no-feedback, exit immediately after test and artifact generation
        if no_feedback:
            print("\n[INFO] Test execution complete. Artifacts and summary generated. Exiting (no feedback mode).")
        ...
        while True:
            ...
            if iteration == 1:
                ...
                print(f"FEEDBACK COLLECTION - After Retry {iteration - 1}")
            print("="*80)
            
            # 🔥 Show different message based on test status
            if final_results['overall_status'] == 'PASSED':
                print("\n✅ Test PASSED - Please review for false positives:")
                print("   - Were any selectors incorrect but still passed?")
                print("   - Did any steps click wrong elements that happened to work?")
                print("   - Should any step use a more specific selector?")
            else:
                print("\n❌ Test FAILED - Please provide corrections:")
                print("   - Which steps used wrong selectors?")
                print("   - What are the correct selectors?")
            ...
            try:
                ...
                print(f"\n⚠️ Feedback collection error: {e}")
                feedback_collected = False

            # Ask user if they want to retry (even if passed)
            if feedback_collected:
                print("\n" + "="*80)
                try:
                    if final_results['overall_status'] == 'PASSED':
                        user_choice = input("\nRetry test with corrected selectors? (y/n): ").strip().lower()
                    else:
                        user_choice = input("\nRetry test with learned insights? (y/n): ").strip().lower()
                ...
                if user_choice in ['y', 'yes']:
                    iteration += 1
                    print("\n" + "="*80)
                    print(f"Test Retry - Iteration {iteration}")
                    print("="*80 + "\n")
                    
                    # Execute test again with learned insights
                    try:
                        final_results = assistant.execute_test(ticket_id)
                    except Exception as e:
                        print(f"\n❌ Test retry failed: {e}")
                        import traceback
                        traceback.print_exc()
                ...
                print("\n[INFO] No feedback provided. Exiting.")
                break
        ...
        print(f"Report: {final_results.get('report_path', 'N/A')}")
        print("="*80)
...
def load_selector_corrections(ticket_id, step_number, step_text):
    ...
    for file in feedback_dir.glob(f"{ticket_id}_step{step_number}_*.json"):
        with open(file, "r", encoding="utf-8") as f:
            data = json.load(f)
            if data["step"] == step_text:
                return data["selector"]
    return None

def get_corrected_selector(ticket_id, step_number):
    # PENDING_DIR = "C:/Idea Projects/AI_Test_Assist/insights/pending"
    
    # PENDING_DIR = "C:/AssureX/backend/insights/pending"
    PENDING_DIR = os.path.join(os.path.dirname(__file__), "insights", "pending")
    pattern = os.path.join(PENDING_DIR, f"{ticket_id}_step{step_number}_*.json")
    ...
    return None
# selector = get_corrected_selector(ticket_id, step_number)
# if selector:
#     print(f"Using corrected selector from feedback: {selector}")
# else:
#     print("Using default selector")
...
def search_pending_insights_embeddings(azure_client, config: Dict[str, Any],
    ...
    try:
        ...
        for candidate in filtered[:max_results]:
            ...
            # boosted_confidence = min(similarity + confidence_boost, 1.0)
            boosted_confidence = 1.0  # ✅ Override calculated confidence

            matches.append({
                'selector': insight.get('selector', ''),
                'confidence': boosted_confidence,
                # 'confidence': 1.0,  # 🔥 FIX: Force to 1.0 (user feedback always wins)
...
def post_steps_to_backend(execution_id, step_results):
    url = "http://localhost:8000/api/save-steps"
    payload = {
    ...
    try:
        ...
        print(f"Posted steps to backend: {resp.status_code} {resp.text}")
    except Exception as e:
        print(f"Failed to post steps to backend: {e}")
    

def run_from_external(ticket_id: str):
    """
    Entry point used by UI.
    Behaves EXACTLY like CLI execution.
    """
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python plcd_taseq.py <TICKET_ID> [--no-feedback|--process-feedback [--rerun]]")
        print("\nExamples:")
        print("  python plcd_taseq.py RBPLCD-8835                              # Interactive mode with feedback loop")
        print("  python plcd_taseq.py RBPLCD-8835 --no-feedback                # Single run, no feedback")
        print("  python plcd_taseq.py RBPLCD-8835 --process-feedback 'text'    # Process feedback and rerun")
        print("  python plcd_taseq.py RBPLCD-8835 --process-feedback 'text' --rerun  # Rerun with visible browser")
        sys.exit(1)

    ticket_id = sys.argv[1]
    
    # 🔥 Detect execution mode flags
    IS_PROCESS_FEEDBACK = "--process-feedback" in sys.argv
    IS_RERUN = "--rerun" in sys.argv
    IS_NO_FEEDBACK = "--no-feedback" in sys.argv
    
    # ============================================================================
    # MODE 1: Process feedback and rerun (called from backend /api/rerun-with-feedback)
    # ============================================================================
    if IS_PROCESS_FEEDBACK:
        print("\n[MODE] Processing feedback and rerunning...")
        
        # Extract feedback text from command line args
        feedback_text = None
        try:
            feedback_index = sys.argv.index("--process-feedback")
            if feedback_index + 1 < len(sys.argv):
                next_arg = sys.argv[feedback_index + 1]
                # Only use next arg if it's not another flag
                if not next_arg.startswith("--"):
                    feedback_text = next_arg
        except (ValueError, IndexError):
            pass
        
        # Load config and set rerun mode
        config = load_config()
        config["rerun_mode"] = True  # 🔥 Forces browser to be visible
        assistant = PLCDTestingAssistantSeq(config)
        
        # =====================================================
        # 1️⃣ PROCESS FEEDBACK (Only if feedback text provided and not already embedded)
        # =====================================================
        if feedback_text:
            try:
                summaries_folder = Path("Reports/summaries")
                ...
                if summary_path.exists():
                    ...
                    # Check if summary has valid step_results
                    if not results.get('step_results'):
                        print(f"⚠️ Summary is empty, will fetch canonical steps from Jira during feedback processing")
                    
                    print(f"📝 Processing feedback: {feedback_text[:50]}...")
                    feedback_collected = collect_and_process_feedback(
                        ticket_id, results, config, assistant, 
                        feedback_text=feedback_text
                    )
                    
                    if feedback_collected:
                        print("✅ Feedback processed and embedded.")
                    else:
                        print("⚠️ Feedback already embedded or skipped.")
                else:
                    print(f"⚠️ No previous summary found at {summary_path}")
                    print("ℹ️ Proceeding with test rerun anyway...")
            
            except Exception as e:
                print(f"⚠️ Could not process feedback: {e}")
                import traceback
                traceback.print_exc()
                print("ℹ️ Proceeding with test rerun anyway...")
        else:
            print("ℹ️ No feedback text provided, proceeding directly to test execution...")
        
        # =====================================================
        # 2️⃣ ALWAYS RUN PLAYWRIGHT TEST (THIS IS THE RERUN!)
        # =====================================================
        print("\n" + "="*80)
        if IS_RERUN:
            print("🔁 RERUNNING TEST WITH LEARNED INSIGHTS (Browser Visible)")
        else:
            print("🔁 RERUNNING TEST WITH LEARNED INSIGHTS")
        print("="*80 + "\n")
        ...
        try:
            ...
            print("\n" + "="*80)
            print(f"✅ Test rerun completed: {results['overall_status']}")
    ...
    # ============================================================================
    # MODE 2: Single execution without feedback (called from backend /api/execute-test)
    # ============================================================================
    elif IS_NO_FEEDBACK:
        print("\n[MODE] Single execution (no feedback)")
        config = load_config()
        assistant = PLCDTestingAssistantSeq(config)
        ...
        try:
            ...
            # Optional: Post steps to backend (if backend endpoint exists)
            try:
                execution_id = f"exec_{ticket_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
                step_results = []
                for step in results.get("step_results", []):
                    step_results.append({
                        "step_num": step.get("step_number") or step.get("step") or 0,
                        "step_text": step.get("step_text") or step.get("text") or "",
                        "status": step.get("status"),
                        "selector_used": step.get("selector", ""),
                        "agent_used": step.get("agent_used", ""),
                        "confidence": step.get("confidence", 0.0),
                        "action_type": step.get("action_type", ""),
                        "screenshot_path": step.get("screenshot_path", None)
                    })
                
                post_steps_to_backend(execution_id, step_results)
            except Exception as post_error:
                logger.warning(f"Could not post steps to backend: {post_error}")
            
            print("\n[INFO] Execution complete. Artifacts and summary generated.")
    ...
    # ============================================================================
        ...
        print("\n[MODE] Interactive execution with feedback loop")
        config = load_config()
        assistant = PLCDTestingAssistantSeq(config)
        ...
        while True:
            ...
            if iteration == 1:
                ...
                print(f"RERUN {iteration} (with learned insights)")
            print(f"{'='*80}\n")
        ...
        # Execute test
            try:
                ...
                print(f"\n{'='*80}")
                print(f"Iteration {iteration} complete: {results['overall_status']}")
                print(f"{'='*80}")
        ...
        # 🔥 ALWAYS collect feedback (for both PASSED and FAILED)
            try:
                print("\n📋 Review the HTML report for accuracy:")
                print(f"   {results.get('report_path', 'Reports/*.html')}")
        ...
        # Exit if no feedback provided
            if not feedback_collected:
                print("\n[INFO] No feedback provided. Exiting.")
                break
        
        # Ask if user wants to retry (message changes based on test status)
            try:
                print("\n" + "="*80)
                if results['overall_status'] == 'PASSED':
                    user_choice = input("Retry test with corrected selectors? (y/n): ").strip().lower()
                else:
                    user_choice = input("Retry test with learned insights? (y/n): ").strip().lower()
    ...
    # Final summary after exiting loop
        print(f"\n{'='*80}")
        print(f"FINAL RESULT after {iteration} iteration(s)")
        print(f"{'='*80}")
        print(f"Status: {results['overall_status']}")
        print(f"Report: {results.get('report_path', 'N/A')}")
        print(f"Video: {results.get('video_path', 'N/A')}")
