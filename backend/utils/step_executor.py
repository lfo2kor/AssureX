...
class StepExecutor:
    ...
    def execute_step(self, step: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            if success:
                ...
                self.logger.error("")
                self.logger.error("L1 (Custom Selectors): FAILED - No matching selector in JSON")
                self.logger.error("L2 (Generic Patterns): FAILED - No generic pattern matched")
                self.logger.error("L3 (CV-Guided): FAILED - CV could not find element")
                self.logger.error("")
                self.logger.error("="*80)
                self.logger.error("ACTION REQUIRED:")
                self.logger.error("="*80)
                self.logger.error("1. Check the failure screenshot below")
                self.logger.error("2. Add missing selector to JSON file, OR")
                self.logger.error("3. Run: python extract_runtime_selectors.py <TICKET_ID>")
                self.logger.error("")
                self.logger.error("="*80)
                self.logger.error("TEST TERMINATED - Fix above issue and restart")
                self.logger.error("="*80)

            # Take screenshot after (even if failed, for debugging)
    ...
    def _execute_three_level_strategy(
        ...
        # This ensures state transitions happen even if L1 is skipped
    ...
    def _try_level1_custom_selectors(self, step_text: str) -> tuple:
        """
        Level 1: Try custom selectors from enriched selectors JSON.
        ...
        try:
            ...
            if not is_dynamic:
                ...
                if possible_values:
                    ...
                    self.logger.info(f"L1: Dynamic selector with {len(possible_values)} possible values")

                    for value in possible_values:
                        selector_str = self.selector_loader.build_selector(selector_obj, value_override=value)
                        self.logger.info(f"L1: Trying dynamic selector with value '{value}': {selector_str}")

                        count = self.page.locator(selector_str).count()
                        self.logger.info(f"L1: Count for value '{value}': {count}")

                        if count > 0:
                            # Found match with this value
                            self.logger.info(f"L1: SUCCESS with possibleValue '{value}'")
                            return self._execute_action(step_text, selector_str)

                    self.logger.info("L1: No possible values matched")
                    return (False, "")

                else:
                    # DYNAMIC WITHOUT VALUES - Try wildcard
                    selector_str = self.selector_loader.build_selector(selector_obj)
                    self.logger.info(f"L1: Trying dynamic selector (wildcard): {selector_str}")

                    count = self.page.locator(selector_str).count()
                    self.logger.info(f"L1: Dynamic wildcard count: {count}")

                    if count == 1:
                        # Unique match with wildcard
                        return self._execute_action(step_text, selector_str)
                    elif count > 1:
                        self.logger.warning(f"L1: Multiple matches ({count}) with wildcard - ambiguous, skipping to Level 3")
                        return (False, "")
                    else:
                        self.logger.info("L1: Dynamic wildcard not found on page")
                        return (False, "")
        ...
        try:
            ...
            if 'select' in step_lower and has_dropdown:
                ...
                if dropdown_value:
                    ...
                    # Check if this is a "create dialog" dropdown (opens modal, not dropdown panel)
