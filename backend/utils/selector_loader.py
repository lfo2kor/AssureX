...
class SelectorLoader:
    ...
    def load_selectors(self):
        ...
        try:
            ...
            # New format: object with metadata + selectors array
    ...
    def build_selector(self, selector_obj: Dict[str, Any], value_override: str = None) -> str:
        ...
        # OLD format: "attr": "attr.data-X" (BROKEN)
        # NEW format: "attr": "data-X" (CORRECT)

        # Handle old format with 'attr.' prefix (for backward compatibility)
    ...
    def find_best_selector(
        ...
        if matches:
            # Sort by priority (higher priority first) and return best match
            sorted_matches = sorted(matches, key=lambda s: s.get('priority', 0), reverse=True)
