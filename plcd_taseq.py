...
# ===============================
    ...
    def _execute_steps_with_agents(
        ...
        for idx, step_data in enumerate(steps):
            ...
            try:
                ...
                # ------------------------------------------------------------------
                # SELECTOR DISCOVERY (Learning -> L1 -> L2 -> L3 fallback)
                # ------------------------------------------------------------------
                # First try learning agent (semantic stored selectors) - may be None
                learned = None
                try:
                    learned = self.learning_agent.query_learned_selector(clean_step_text, context)
                except Exception as e:
                    logger.debug(f"LearningAgent query failed: {e}")

                if learned and learned.get("confidence", 0) > 0.90:
                    selector = learned["selector"]
                    confidence = learned["confidence"]
                    agent_used = "Learning"
                    logger.info(f"[Learning] Using selector: {selector} (conf: {confidence:.2f})")
                else:
                    # L1 discovery (RAG + LLM)
                ...
                if success and selector:
                    try:
                        self.learning_agent.store_learned_selector(clean_step_text, selector, context, confidence, source="runtime", verified=True)
                    except Exception as e:
    ...
    #     clean_step_text = step_text  # No modifications needed if Jira parsing is clean
        
    #     # When saving to state:
    #     state["step_results"][idx]["step_text"] = clean_step_text  # NOT step_text + console output

    #     state["current_step"] = step_number
    #     idx = step_number - 1  # 🔑 CRITICAL INDEX

    #     # Display clean step text
    ...
    #     # LOGIN SKIP
    #     # ------------------------------------------------------------------
    #     if step_text.lower() in ["login", "log in", "sign in", "signin"]:
    #         if state.get("logged_in", False):
    #             print(" [SKIPPED]")

    #             state["step_results"][idx]["status"] = "SKIPPED"
    #             state["step_results"][idx]["confidence"] = 1.0
    #             state["step_results"][idx]["agent_used"] = "Auth"
    #             state["step_results"][idx]["action_type"] = "login"
    ...
    #             state["step_results"][idx]["status"] = "PASSED" if success else "FAILED"
    #             state["step_results"][idx]["confidence"] = confidence
    #             state["step_results"][idx]["agent_used"] = "L3"
    #             state["step_results"][idx]["action_type"] = "verify_text"
    ...
    #         if not selector or confidence < 0.70:
    #             print(f"  [L1 Low Confidence: {confidence:.2f}] Trying L2...")
    ...
    #             if not selector or confidence < 0.70:
    #                 raise Exception("No selector found")
    ...
    #         state["step_results"][idx]["status"] = "PASSED" if success else "FAILED"
    #         state["step_results"][idx]["confidence"] = confidence
    #         state["step_results"][idx]["agent_used"] = agent_used
    #         state["step_results"][idx]["action_type"] = action_type
    #         state["step_results"][idx]["selector"] = selector
    ...
    #         state["step_results"][idx]["status"] = "FAILED"
    #         state["step_results"][idx]["confidence"] = 0.0
    #         state["step_results"][idx]["agent_used"] = "ERROR"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python plcd_taseq.py <TICKET_ID> [--no-feedback|--process-feedback]")
        sys.exit(1)

    ticket_id = sys.argv[1]
    
    # MODE 1: Process feedback only (called from rerun-with-feedback endpoint)
    if len(sys.argv) >= 3 and sys.argv[2] == "--process-feedback":
        print("\n[MODE] Processing feedback and rerunning...")
        feedback_text = sys.argv[3] if len(sys.argv) > 3 else None
        ...
        if not summary_path.exists():
            print(f"[ERROR] No summary found for {ticket_id}")
            sys.exit(1)
        ...
        if feedback_collected:
            print("\n[INFO] Feedback processed. Rerunning test with learned insights...")
            results = assistant.execute_test(ticket_id)
            sys.exit(0 if results['overall_status'] == 'PASSED' else 1)
        else:
    ...
    # MODE 2: Single execution without feedback (called from execute_test_in_background)
    elif '--no-feedback' in sys.argv:
    ...
    # ✅ END BLOCK
        ...
        # ✅ CRITICAL: Script terminates immediately after execution
    ...
    # MODE 3: Interactive feedback loop (manual CLI usage only)
        ...
        # Continuous feedback loop for manual testing
        while True:
            print(f"\n{'='*80}")
        ...
        print(f"\n[INFO] Final status: {results['overall_status']}")
        sys.exit(0 if results['overall_status'] == 'PASSED' else 1)